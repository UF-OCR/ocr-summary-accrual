...
def validate_protocol(username, password, protocol_no):
    ...
    response = requests.get(protocol_request, auth=HTTPBasicAuth(username, password))
...
def post_accruals(data):
    ...
    response = requests.post(accrual_request, data=json.dumps(data), headers=headers)
    return response.status_code, response.content
